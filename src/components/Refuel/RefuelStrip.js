  ...
  return (
    <div className="p-4 w-auto bg-gradient-to-br from-pink-500 to-orange-400 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-pink-200 dark:focus:ring-pink-800 font-medium rounded-lg">
      <div className="flex justify-between items-center mb-4">
        <h5 className="text-xl font-bold leading-none text-white">{props.refuels.length == 0 ? "No Refuel History" : "Refuels"}</h5>
      </div>
                  ...
                  <div className="flex items-center space-x-4">
                    <div className="flex-shrink-0">
                      <div className="inline-flex overflow-hidden relative justify-center items-center w-10 h-10 bg-white-100 rounded-lg">
                        <span className="text-[13px] leading-[.8rem] text-center uppercase text-white  ">{format(new Date(refuel.refuel_on), "D MMM YYYY")} </span>
                      </div>
                    </div>
                    <div className="flex-1 min-w-0">
                      ...
                      {previousRefuel?.meter_reading && refuel.continued && <span className=" text-white  font-medium rounded-sm text-[12px] pr-2 py-1 text-center inline-flex items-center  mr-2 mb-2"> {"Run: " + (refuel.meter_reading - previousRefuel.meter_reading) + " KM"} </span>}
                      {refuel.rate_per_litre && <span className=" text-white  font-medium rounded-sm text-[12px] pr-2 py-1 text-center inline-flex items-center  mr-2 mb-2"> {"Fuel Rate: Rs. " + parseFloat(refuel.rate_per_litre).toFixed(2)} </span>}
                      {refuel.spending && <span className=" text-white  font-medium rounded-sm text-[12px] pr-2 py-1 text-center inline-flex items-center  mr-2 mb-2"> {"Litres: Rs. " + (parseFloat(refuel.spending) / parseFloat(refuel.rate_per_litre)).toFixed(2)} </span>}
                    ...
                    <div className="inline-flex items-center text-base font-semibold font-mono text-gray-900 dark:text-white">{props.vehicle.currency} {parseFloat(refuel.spending).toFixed(2)}</div>
                  </div>
                </li>
