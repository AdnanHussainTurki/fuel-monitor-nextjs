...
export default function Refuel() {
  ...
  const submitHandler = async (event) => {
    ...
    // Add validation
    const data = await doRefuel(vid,spendingInputRef.current.value,meterReadingInputRef.current.value,ratePerLitreInputRef.current.value,refuelingOnInputRef.current.value,fuelPercentBeforeRefuelInputRef.current.value);
    setIsAdding(false);
          ...
          <div className="p-6  bg-white border-b ">
            ...
            <form onSubmit={submitHandler}>
              ...
              <div>
                ...
                <div className="relative mb-1">
                  ...
                  <input
                    dir="rtl"
                    id="model"
                    min={(lastRefuel.meter_reading)? lastRefuel.meter_reading : 1}
                    className=" appearance-none text-xl font-bold caret-transparent text-red-600 caret-white rounded-md cursor-pointer shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 block mt-1 p-2 w-full"
                    required="required"
                    autoComplete="off"
                    type="number"
              ...
              <div className="text-[9px] text-gray-500 font-italic text-right">
                <span className="text-grey-600 uppercase">Last Meter Reading:</span>{" "}
                <span className="text-red-600">{lastRefuel.meter_reading}</span>
              </div>
              <div className="mt-3">
                <label
                  className="block text-xs font-normal uppercase  text-red-600 text-right"
              ...
              <div className="text-[9px] text-gray-500 font-italic text-right">
                <span className="text-grey-600 uppercase">Last FUEL RATE:</span>{" "}
                <span className="text-red-600">{lastRefuel.rate_per_litre}</span>
              </div>
              <div className="mt-3">
                <label
                  className="block text-xs font-normal uppercase  text-red-600 text-right"
              ...
              <div className="text-[9px] text-gray-500 font-italic text-right">
                <span className="text-grey-600 uppercase">Last REFUEL Date:</span>{" "}
                <span className="text-red-600">{lastRefuel.refuel_on}</span>
              </div>
              <div className="mt-3">
                <label
                  className="block text-xs font-normal uppercase  text-red-600 text-right"
